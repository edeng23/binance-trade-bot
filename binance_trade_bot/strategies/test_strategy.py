...
class Strategy(AutoTrader):
    ...
    def scout(self):
        ...
        if base_time >= self.panic_time and not self.panicked:
            ...
            win_threshold = max(min(((1+self.win/balance_in_bridge)**(1/self.jumps) - 1)*100, 2**(1/self.jumps)), ((st.stdev(numpy.array(self.from_coin_prices)) * 3 + self.mean_price) / (self.mean_price) - 1) * 100)
