...
def get_currency_balance(client: Client, currency_symbol: str):
    ...
    """
    for currency_balance in client.get_account()["balances"]:
...
def scout(client: Client, transaction_fee=0.001, multiplier=5):
    ...
    if ratio_dict:
        ...
        logger.info(
            "Will be jumping from {0} to {1}".format(current_coin, best_pair.to_coin_id)
        )
        transaction_through_tether(client, best_pair)
...
def main():
    ...
    schedule.every(SCOUT_SLEEP_TIME).seconds.do(
        ...
        multiplier=SCOUT_MULTIPLIER,
    ).tag("scouting")
    schedule.every(1).minutes.do(update_values, client=client).tag(
