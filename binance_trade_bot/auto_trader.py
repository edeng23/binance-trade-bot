...
class AutoTrader:
    ...
    def transaction_through_bridge(self, pair: Pair, all_tickers):
        ...
        if self.manager.sell_alt(pair.from_coin, self.config.BRIDGE) is None:
            ...
            return None
        # This isn't pretty, but at the moment we don't have implemented logic to escape froma
        # bridge coin... This'll do for now
        result = None
    ...
    def initialize_trade_thresholds(self):
        ...
        with self.db.db_session() as session:
            for pair in (
                session.query(Pair)
                .filter(Pair.ratio == None)  # pylint: disable=singleton-comparison
                .all()
            ):
    ...
    def initialize_current_coin(self):
        ...
        if self.db.get_current_coin() is None:
            ...
            if current_coin_symbol not in self.config.SUPPORTED_COIN_LIST:
                sys.exit(
                    "***\nERROR!\nSince there is no backup file, a proper coin name "
                    "must be provided at init\n***"
                )
            self.db.set_current_coin(current_coin_symbol)

            # if we don't have a configuration, we selected a coin at random... Buy it so we can
            # start trading.
    ...
    def scout(self):
        ...
        current_coin = self.db.get_current_coin()
        # Display on the console, the current coin+Bridge, so users can see *some* activity and not
        # thinkg the bot has stopped. Not logging though to reduce log size.
        print(
            str(datetime.now())
        ...
        current_coin_price = get_market_ticker_price_from_list(
            all_tickers, current_coin + self.config.BRIDGE
        )
