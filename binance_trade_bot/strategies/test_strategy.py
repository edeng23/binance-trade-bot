...
class Strategy(AutoTrader):
    ...
    def scout(self):
        ...
        if base_time >= allowed_rsi_time:
            ...
            if len(sp_prices) >= 2 and not self.panicked:
        ...
        print(
            ...
            f"Panic if zero: {(self.slope):.3E} " if self.slope and not self.panicked else "",
        ...
        if base_time >= panic_time and not self.panicked:
            ...
            self.panic_time = self.manager.now().replace(second=0, microsecond=0) + timedelta(minutes=1)
            if self.mean_price <= self.from_coin_prices[-1]:
